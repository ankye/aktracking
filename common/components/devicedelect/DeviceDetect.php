...
class DeviceDetect extends MobileDetect
{
    ...
    public function devideType($userAgent = null)
    {
        if($this->isDesktop($userAgent)) return 1;
        if($this->isPhone($userAgent))  return 2;
        if($this->isTablet($userAgent)) return 3;
        if($this->isRobot($userAgent)) return 4;
    }
    /**
     * @inheritdoc
     */
